...
from scalesim.memory.double_buffered_scratchpad_mem import (
    double_buffered_scratchpad as mem_dbsp,
)
...
class single_layer_sim:
    ...
    def set_params(
        self, layer_id=0, config_obj=cfg(), topology_obj=topo(), verbose=True
    ):
    ...
    def run(self):
        ...
        self.num_compute = self.topo.get_layer_num_ofmap_px(
            self.layer_id
        ...
        (
            ifmap_prefetch_mat,
            filter_prefetch_mat,
        ) = self.compute_system.get_prefetch_matrices()
        (
            ifmap_demand_mat,
            filter_demand_mat,
            ofmap_demand_mat,
    ...
    def calc_report_data(self):
        ...
        self.overall_util = (self.num_compute * 100) / (
            self.total_cycles * self.num_mac_unit
        )
    ...
    def get_bandwidth_report_items(self):
        ...
        items = [
            self.avg_ifmap_sram_bw,
            self.avg_filter_sram_bw,
            self.avg_ofmap_sram_bw,
        ]
        items += [
            self.avg_ifmap_dram_bw,
            self.avg_filter_dram_bw,
            self.avg_ofmap_dram_bw,
        ]
